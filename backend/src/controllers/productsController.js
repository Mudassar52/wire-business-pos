...
// Purchased/sold quantities (and true weighted-average purchase cost) per
// product, computed straight from the DB so stock — and cost basis — is
// always derived from real purchase + sale history, never a stored/manual
// field. avgCost = total(qty*rate - discount) / total(qty) across every
// purchase of that product; this is what actually determines gross profit
// on a sale (see salesController.weightedAvgCost), so it's surfaced here
// too, letting the frontend preview the same number instead of reading
// products.purchase_price and drifting out of sync with it.
async function inventoryMap(ownerId) {
  const purchasedQ = query(
    `select product_id, coalesce(sum(quantity_kg),0) as qty,
            coalesce(sum(quantity_kg * purchase_rate - coalesce(discount_amount,0)),0) as cost
  ...
  const map = new Map();
  for (const r of purchased.rows) {
    const qty = Number(r.qty);
    const cost = Number(r.cost);
    map.set(r.product_id, { purchased: qty, sold: 0, avgCost: qty > 0 ? cost / qty : null });
  }
  for (const r of sold.rows) {
    const cur = map.get(r.product_id) || { purchased: 0, sold: 0, avgCost: null };
...
export const listProducts = asyncHandler(async (req, res) => {
  ...
  const withStats = rows.map((row) => {
    ...
    const stats = inv.get(row.id) || { purchased: 0, sold: 0, avgCost: null };
    const remaining = Math.max(0, stats.purchased - stats.sold);
    // Fall back to the manual purchasePrice only when the product has no
    // purchase history yet at all (avgCost is null) — e.g. a brand-new
    // product added before its first purchase invoice.
    const avgCost = stats.avgCost ?? product.purchasePrice;
    return {
      ...product,
      avgCost,
      ...
      profit: remaining * (product.salePrice - avgCost),
    };
  });
