...
// True cost basis for a product: the weighted-average rate across every
// purchase ever recorded for it (quantity*rate, net of any supplier
// discount, divided by total quantity). This is computed fresh from the
// `purchases` table every time — never from products.purchase_price, which
// is just a manually-typed field on the Products page that the owner can
// forget to update. Any purchaseRate the client sends for a sale line is
// therefore advisory only (used for an instant estimate in the POS UI
// before the request round-trips); the figure actually persisted to
// sales/sale_lines always comes from here, so gross/net profit can never
// drift out of sync with what was really paid for the stock.
...
async function costMapFor(client, ownerId, productIds) {
  const map = new Map();
  for (const productId of productIds) {
    map.set(productId, await weightedAvgCost(client, ownerId, productId));
  }
  return map;
}
...
export const completeSale = asyncHandler(async (req, res) => {
  ...
  const result = await withTransaction(async (client) => {
    ...
    // Overwrite whatever purchaseRate the client sent with the real,
    // server-computed weighted-average cost (see weightedAvgCost above).
    const costMap = await costMapFor(client, req.user.id, [...neededByProduct.keys()]);
    for (const l of body.lines) l.purchaseRate = costMap.get(l.productId) ?? 0;
    ...
    if (body.paymentMethod === "credit" && customerId) {
      // Previously this stored `subtotal - paidAmount` (the net debt) as the
      // only ledger entry, which silently hid any advance the customer paid
      // at the counter: the customer's ledger showed a smaller "Billed"
      // figure than the real invoice, and never showed a "Received" entry
      // for that upfront cash at all. Store the true billed amount here...
      await client.query(
        `insert into credit_transactions (owner_id, customer_id, sale_id, type, amount, date, note)
         values ($1,$2,$3,'sale',$4,$5,'Credit sale')`,
        [req.user.id, customerId, sale.id, subtotal, body.date]
      );
      // ...and record the advance as its own payment entry so it shows up in
      // the ledger's "Received" column against this exact invoice.
      if (body.paidAmount > 0) {
        await client.query(
          `insert into credit_transactions (owner_id, customer_id, sale_id, type, amount, date, note)
           values ($1,$2,$3,'payment',$4,$5,'Advance received at time of sale')`,
          [req.user.id, customerId, sale.id, body.paidAmount, body.date]
        );
      }
    }
...
export const editSale = asyncHandler(async (req, res) => {
  ...
  const result = await withTransaction(async (client) => {
    ...
    // Overwrite whatever purchaseRate the client sent with the real,
    // server-computed weighted-average cost — same as completeSale above.
    const costMap = await costMapFor(client, req.user.id, [...neededByProduct.keys()]);
    for (const l of body.lines) l.purchaseRate = costMap.get(l.productId) ?? 0;
    ...
    // The upfront advance is its own ledger row (see completeSale above),
    // tagged with this fixed note so we can find and update just that one
    // entry here without disturbing any other payments recorded against
    // this sale later (which use different notes).
    const existingAdvanceTx = await client.query(
      "select * from credit_transactions where sale_id = $1 and type = 'payment' and note = 'Advance received at time of sale'",
      [saleId]
    );
    if (paymentMethod === "credit" && customerId) {
      // Ledger 'sale' entry always carries the true billed amount (subtotal),
      // never netted against what's been paid — see completeSale for why.
      if (existingDebtTx.rows[0]) {
        await client.query("update credit_transactions set amount = $1, customer_id = $2 where id = $3", [subtotal, customerId, existingDebtTx.rows[0].id]);
      } else {
        await client.query(
          `insert into credit_transactions (owner_id, customer_id, sale_id, type, amount, date, note)
           values ($1,$2,$3,'sale',$4,$5,'Credit sale')`,
          [req.user.id, customerId, saleId, subtotal, sale.date]
        );
      }
      if (paidAmount > 0) {
        if (existingAdvanceTx.rows[0]) {
          await client.query("update credit_transactions set amount = $1, customer_id = $2 where id = $3", [paidAmount, customerId, existingAdvanceTx.rows[0].id]);
        } else {
          await client.query(
            `insert into credit_transactions (owner_id, customer_id, sale_id, type, amount, date, note)
             values ($1,$2,$3,'payment',$4,$5,'Advance received at time of sale')`,
            [req.user.id, customerId, saleId, paidAmount, sale.date]
          );
        }
      } else if (existingAdvanceTx.rows[0]) {
        await client.query("delete from credit_transactions where id = $1", [existingAdvanceTx.rows[0].id]);
      }
    } else {
      if (existingDebtTx.rows[0]) await client.query("delete from credit_transactions where id = $1", [existingDebtTx.rows[0].id]);
      if (existingAdvanceTx.rows[0]) await client.query("delete from credit_transactions where id = $1", [existingAdvanceTx.rows[0].id]);
    }
...
export const getSalePaymentInfo = asyncHandler(async (req, res) => {
  ...
  // sale.paid_amount is the only record of what was paid at checkout for
  // CASH sales (they never get credit_transactions rows), so it must always
  // be counted. The advance-at-checkout ledger entry (credit sales only)
  // duplicates sale.paid_amount, so exclude it here to avoid counting it twice.
  const payments = allPayments.filter((p) => p.note !== "Advance received at time of sale");
  ...
  res.json({ ok: true, payments, totalPaid, remaining });
});
