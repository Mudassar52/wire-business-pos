...
export const getDashboard = asyncHandler(async (req, res) => {
  ...
  const [revenueQ, purchaseQ, expenseQ, creditQ, stockQ] = await Promise.all([
    query(
      "select coalesce(sum(subtotal),0) as revenue, count(*) as sales_count from sales where owner_id = $1 and date between $2 and $3",
      [ownerId, from, to]
    ),
    query(
      "select coalesce(sum(quantity_kg * purchase_rate - coalesce(discount_amount,0)),0) as spend, coalesce(sum(quantity_kg),0) as qty, count(*) as purchase_count from purchases where owner_id = $1 and date between $2 and $3",
      [ownerId, from, to]
    ),
    query(
    ...
    query(
      `select p.id, p.purchase_price, p.sale_price,
         ...
         coalesce((select sum(quantity_kg * purchase_rate - coalesce(discount_amount,0)) from purchases pu where pu.product_id = p.id and pu.owner_id = $1),0) as purchase_cost,
  ...
  const purchaseCount = Number(purchaseQ.rows[0].purchase_count);
  ...
  const grossProfit = revenue - purchaseSpend;
  ...
  for (const row of stockQ.rows) {
    ...
    const purchasedQty = Number(row.purchased);
    // Weighted-average actual cost across every purchase of this product,
    // same figure the sales controller uses — never the manually-typed
    // products.purchase_price, which can silently drift out of sync (see
    // salesController.weightedAvgCost for the full rationale). Only fall
    // back to purchase_price for a product with zero purchase history.
    const avgCost = purchasedQty > 0 ? Number(row.purchase_cost) / purchasedQty : Number(row.purchase_price);
    stockCost += remaining * avgCost;
