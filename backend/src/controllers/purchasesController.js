...
export const createPurchase = asyncHandler(async (req, res) => {
  ...
  let row = rows[0];
  // Invoice number is optional on the form (e.g. a supplier who just drops
  // off free stock with no paperwork). If left blank, auto-generate one from
  // the new row's id so it's never empty in the database itself, not just on
  // screen — PUR-<id> matches what the UI already shows as a fallback.
  if (!row.invoice_number) {
    const auto = `PUR-${row.id}`;
    const updated = await query(
      "update purchases set invoice_number = $1 where id = $2 returning *",
      [auto, row.id]
    );
    row = updated.rows[0];
  }
  res.status(201).json({ ok: true, message: "Purchase added and inventory updated", purchase: toPurchase(row) });
});
...
export const updatePurchase = asyncHandler(async (req, res) => {
  ...
  if (!row.invoice_number) {
    const auto = `PUR-${row.id}`;
    const updated = await query("update purchases set invoice_number = $1 where id = $2 returning *", [auto, row.id]);
    row = updated.rows[0];
  }
  res.json({ ok: true, message: "Purchase updated", purchase: toPurchase(row) });
});
