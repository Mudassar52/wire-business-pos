...
export const createPurchase = asyncHandler(async (req, res) => {
  ...
  const { rows } = await query(
    `insert into purchases (owner_id, supplier_id, invoice_number, date, product_id, quantity_kg, purchase_rate, paid_amount, notes)
     values ($1,$2,$3,$4,$5,$6,$7,$8,$9) returning *`,
    [req.user.id, p.supplierId, p.invoiceNumber, p.date, p.productId, p.quantityKg, p.purchaseRate, p.paidAmount, p.notes]
  );
...
export const updatePurchase = asyncHandler(async (req, res) => {
  ...
  const colMap = { supplierId: "supplier_id", invoiceNumber: "invoice_number", date: "date", productId: "product_id", quantityKg: "quantity_kg", purchaseRate: "purchase_rate", paidAmount: "paid_amount", notes: "notes" };
  const fields = [];
  const values = [];
  let i = 1;
...
export const payPurchaseInvoice = asyncHandler(async (req, res) => {
  ...
  const result = await withTransaction(async (client) => {
    ...
    const total = Number(purchase.quantity_kg) * Number(purchase.purchase_rate);
