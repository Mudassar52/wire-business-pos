...
async function start() {
  ...
  // Opt-in, one-time-use safety valve for hosts (like Render's free plan)
  // that don't offer a Shell: set AUTO_SEED_SUPERADMIN=true in your
  // environment variables, redeploy once, check the logs for the created
  // email/password, then REMOVE this variable (or set it back to false) so
  // it doesn't keep running on every restart.
  if (process.env.AUTO_SEED_SUPERADMIN === "true") {
    try {
      await ensureSuperAdmin();
    } catch (err) {
      // eslint-disable-next-line no-console
      console.error("❌ AUTO_SEED_SUPERADMIN failed:", err);
    }
  }
