...
create table if not exists products (
  ...
  purchase_price  numeric(20,12) not null default 0,
  sale_price      numeric(20,12) not null default 0,
...
create table if not exists purchases (
  ...
  purchase_rate   numeric(20,12) not null,
...
create table if not exists sale_lines (
  ...
  product_id    bigint references products(id) on delete restrict,
  quantity_kg   numeric(14,3) not null,
  sale_rate     numeric(20,12) not null,
  purchase_rate numeric(20,12) not null,
  date          date
);
...
-- saving), so a per-Ton value typed with several decimal places — e.g.
-- 892.0843346 — becomes a per-Kg value that needs the same number of extra
-- decimal places (0.8920843346) to round-trip exactly. These columns used
-- to be too narrow (first numeric(14,2), then numeric(14,6)), which
-- silently rounded that down and showed a wrong, truncated rate after every
-- refresh. numeric(20,12) gives 12 decimal places on the stored per-Kg
-- value — 9 decimal places of precision on the per-Ton rate the user
-- actually types, comfortably inside what a JS double can hold exactly for
-- numbers in this range. Widening the columns here fixes it for both new
-- and already-existing databases — `alter column ... type` is a no-op once
-- the column already matches, so this is safe to run on every startup.
alter table products alter column purchase_price type numeric(20,12);
alter table products alter column sale_price type numeric(20,12);
alter table purchases alter column purchase_rate type numeric(20,12);
alter table sale_lines alter column sale_rate type numeric(20,12);
alter table sale_lines alter column purchase_rate type numeric(20,12);
