...
-- Safe to re-run on a database created before the `discount` column existed.
alter table if exists sales add column if not exists discount numeric(14,2) not null default 0;
