...
import { BusinessProvider, dateLabel, inventoryStats, money, qty, saleCustomerLabel, salePaymentInfo, useBusiness, type Product, type SaleLine } from "./business";
...
// Drop your logo file at public/brand/logo.png (see public/brand/README.txt).
// Until that file exists, this falls back to the default icon automatically.
function BrandLogo({className="",iconSize=19,icon:Icon=Zap}:{className?:string;iconSize?:number;icon?:Icon}){
  const [failed,setFailed]=useState(false);
  if(failed) return <div className={`grid shrink-0 place-items-center rounded-lg bg-emerald-400 text-slate-900 ${className}`}><Icon size={iconSize} fill="currentColor"/></div>;
  return <img src="/brand/logo.png" alt="Business logo" className={`shrink-0 rounded-lg object-contain ${className}`} onError={()=>setFailed(true)} data-testid="img-brand-logo"/>;
}
  ...
  return <Popover open={open} onOpenChange={setOpen}>
    ...
    <PopoverContent align="end" alignOffset={-4} sideOffset={10} className="z-[60] w-64 p-0" data-testid="popover-profile-menu">
      ...
      <div className="p-2">
        ...
        {user?.role!=="superadmin"&&<button type="button" onClick={()=>setLoc("/settings")} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm font-semibold text-slate-700 hover:bg-slate-100" data-testid="link-open-settings"><SettingsIcon size={15}/> Settings</button>}
...
function QuantityInput({kgValue,onChangeKg,testIdPrefix,label="Quantity"}:{kgValue:string;onChangeKg:(v:string)=>void;testIdPrefix:string;label?:string}){
  ...
  const displayValue=kgValue===""?"":String(Number((kgNum/KG_PER_TON).toFixed(4)));
  const handleChange=(v:string)=>{if(v===""){onChangeKg("");return}const n=Number(v);if(Number.isNaN(n))return;onChangeKg(String(n*KG_PER_TON))};
  return <div>
    <div className="mb-1"><span className="text-xs font-bold text-slate-600">{label}</span></div>
    <input type="number" min="0" step="any" className="input-field" value={displayValue} onChange={e=>handleChange(e.target.value)} placeholder="e.g. 0.5" data-testid={`input-${testIdPrefix}-quantity`}/>
...
function PriceInput({perKgValue,onChangePerKg,testIdPrefix,label="Rate per Ton"}:{perKgValue:string;onChangePerKg:(v:string)=>void;testIdPrefix:string;label?:string}){
  ...
  const displayValue=perKgValue===""?"":String(Number((perKgNum*KG_PER_TON).toFixed(4)));
  const handleChange=(v:string)=>{if(v===""){onChangePerKg("");return}const n=Number(v);if(Number.isNaN(n))return;onChangePerKg(String(n/KG_PER_TON))};
  return <div>
    <div className="mb-1"><span className="text-xs font-bold text-slate-600">{label}</span></div>
    <input type="number" min="0" step="any" className="input-field" value={displayValue} onChange={e=>handleChange(e.target.value)} placeholder="e.g. 510000" data-testid={`input-${testIdPrefix}-rate`}/>
...
function SuperAdminShell({children}:{children:ReactNode}) {
  ...
  return <div className="app-shell flex">
     <aside className={`app-sidebar drawer-panel fixed inset-y-0 left-0 z-40 ${railW} max-h-dvh flex-col overflow-y-auto overscroll-contain px-3 py-5 transition-[width] duration-200 lg:flex ${open?"!flex":"hidden"}`} aria-label="Super admin navigation">
      <div className={`flex items-center gap-3 px-3 pb-8 ${railCollapsed?"lg:justify-center lg:px-0":""}`}><BrandLogo className="h-9 w-9" iconSize={19} icon={ShieldCheck}/><div className={railCollapsed?"lg:hidden":""}><div className="font-display text-lg font-bold tracking-tight text-emerald-600">Tech Riwaayat</div><div className="text-[10px] font-bold uppercase tracking-[.18em] text-teal-700">Super Admin</div></div><button className="ml-auto text-slate-500 lg:hidden" onClick={()=>setOpen(false)} aria-label="Close menu"><X size={18}/></button></div>
...
function Shell({children}:{children:ReactNode}) {
  ...
  return <div className="app-shell flex">
     <aside className={`app-sidebar drawer-panel desktop-sidebar fixed inset-y-0 left-0 z-40 ${railW} max-h-dvh flex-col overflow-y-auto overscroll-contain px-3 py-5 transition-[width] duration-200 lg:flex ${open?"!flex":"hidden"}`} aria-label="Primary navigation">
      <div className={`flex items-center gap-3 px-3 pb-8 ${railCollapsed?"lg:justify-center lg:px-0":""}`}><BrandLogo className="h-9 w-9" iconSize={19}/><div className={railCollapsed?"lg:hidden":""}><div className="font-display text-lg font-bold tracking-tight text-emerald-600">Tech Riwaayat</div><div className="text-[10px] font-bold uppercase tracking-[.18em] text-teal-700">Business POS</div></div><button className="ml-auto text-slate-500 lg:hidden" onClick={()=>setOpen(false)} aria-label="Close menu"><X size={18}/></button></div>
...
function saleInvoiceData(b:any,sale:any):InvoiceData{
  ...
  const totals:InvoiceData["totals"]=[{label:"Subtotal",value:money(sale.subtotal)},{label:"Paid at time of sale",value:money(sale.paidAmount)}];
  ...
  if(sale.paymentMethod==="credit"&&customer){
    const {balance}=customerBalance(b,customer);
    totals.push({label:balance>0?"Customer overall balance due":"Customer overall balance",value:balance>0?money(balance):balance<0?`${money(-balance)} advance available`:"Fully settled",emphasis:true});
  }
  return {kind:"Sale Invoice",number:`SALE-${String(sale.id).padStart(4,"0")}`,date:dateLabel(sale.date),business:invoiceBusiness(b),partyLabel:"Billed to",partyName,partyPhone,partyAddress,lines:allLines,totals,note:sale.paymentMethod==="credit"?"This is a credit sale. Every payment received against this sale is itemized above; the customer's overall account balance across all sales is shown separately.":"Paid in full at time of sale."};
}
...
function purchaseInvoiceData(b:any,purchase:any,supplier:any):InvoiceData{
  ...
  const total=purchase.quantityKg*purchase.purchaseRate;
  const lines=[{label:p?.name||"Wire product",qty:qty(purchase.quantityKg),rate:`${money(purchase.purchaseRate*KG_PER_TON)}/Ton`,total:money(total)}];
  const totals:InvoiceData["totals"]=[{label:"Invoice total",value:money(total)},{label:"Paid",value:money(purchase.paidAmount)},{label:"Balance",value:money(total-purchase.paidAmount),emphasis:true}];
...
function supplierLedgerEntries(b:any,supplier:any){
  ...
  ps.forEach((x:any)=>{
    const total=x.quantityKg*x.purchaseRate;
...
function Dashboard(){
 ...
 const start=range==="today"?today:range==="week"?new Date(Date.now()-6*86400000).toISOString().slice(0,10):range==="month"?new Date(new Date().getFullYear(),new Date().getMonth(),1).toISOString().slice(0,10):range==="year"?`${new Date().getFullYear()}-01-01`:customFrom;
 const end=range==="custom"?customTo:today;
 const periodLabel=range==="today"?"today":range==="week"?"last 7 days":range==="month"?"this month":range==="year"?"this year":`${dateLabel(start)} – ${dateLabel(end)}`;
 const totals=useMemo(()=>{
  ...
  const gross=periodSales.reduce((a,x)=>a+x.grossProfit,0);
  const expenses=periodExpenses.reduce((a,x)=>a+x.amount,0);
  const stock=b.products.map(p=>inventoryStats(b,p));
  return {sales,gross,expenses,stock,periodSalesCount:periodSales.length,net:gross-expenses,credit:b.creditTransactions.reduce((a,x)=>a+(x.type==="sale"?x.amount:-x.amount),0)};
 },[b,start,end]);
 const chartData=useMemo(()=>{
  ...
  if(dayCount<=31){
   return Array.from({length:dayCount},(_,index)=>{
    ...
    return {day:dayCount<=7?d.toLocaleDateString("en-US",{weekday:"short"}):d.toLocaleDateString("en-US",{day:"2-digit",month:"short"}),revenue:daySales.reduce((a,x)=>a+x.subtotal,0),profit:daySales.reduce((a,x)=>a+x.grossProfit,0)};
   });
  }
...
function POS(){const b=useBusiness();const [cart,setCart]=useState<any[]>([]);const [pid,setPid]=useState("");const [q,setQ]=useState("1000");const qKg=Number(q||0);const [customer,setCustomer]=useState("");const [walkInName,setWalkInName]=useState("");const [walkInPhone,setWalkInPhone]=useState("");const [walkInAddress,setWalkInAddress]=useState("");const [method,setMethod]=useState<"cash"|"credit">("cash");const [paid,setPaid]=useState("");const [saleDate,setSaleDate]=useState(today);const [toast,setToast]=useState("");const selected=b.products.find(p=>p.id===Number(pid));const totals=cart.reduce((a,x)=>({sub:a.sub+x.quantityKg*x.saleRate,profit:a.profit+x.quantityKg*(x.saleRate-x.purchaseRate)}),{sub:0,profit:0});const addLine=()=>{if(!selected||qKg<=0){setToast("Select a product and enter a valid quantity");return}const stock=inventoryStats(b,selected).remaining;const existing=cart.find(x=>x.productId===selected.id)?.quantityKg||0;if(qKg+existing>stock){setToast(`Insufficient stock. Available: ${qty(stock)}`);return}setCart(c=>{const found=c.find(x=>x.productId===selected.id);return found?c.map(x=>x.productId===selected.id?{...x,quantityKg:x.quantityKg+qKg}:x):[...c,{productId:selected.id,quantityKg:qKg,saleRate:selected.salePrice,purchaseRate:selected.purchasePrice}]});setQ("1000")};const complete=async()=>{if(!cart.length){setToast("Add at least one wire to the cart");return}if(method==="credit"&&!customer){setToast("Select a customer for credit sales");return}const result=await b.completeSale({id:0,date:saleDate||today,customerId:customer?Number(customer):null,paymentMethod:method,lines:cart,subtotal:totals.sub,grossProfit:totals.profit,paidAmount:paid===""?(method==="cash"?totals.sub:0):Number(paid)},!customer&&walkInName.trim()?{name:walkInName,phone:walkInPhone,address:walkInAddress}:undefined);setToast(result.message);if(result.ok){setCart([]);setPaid("");setCustomer("");setWalkInName("");setWalkInPhone("");setWalkInAddress("");setSaleDate(today)}};return <><PageHead eyebrow="Counter sales" title="Point of Sale" description="Build a multi-line Ton sale with live stock checks and margin visibility."/><div className="content-grid"><div className="data-card span-7 p-5"><div className="flex items-center justify-between"><div><div className="kicker">Add to ticket</div><h3 className="mt-1 font-display text-lg font-bold">Select wire</h3></div></div><div className="mt-5 grid gap-4 md:grid-cols-[1fr_140px_auto] md:items-end"><Field label="Product"><select className="input-field" value={pid} onChange={e=>setPid(e.target.value)} data-testid="select-pos-product"><option value="">Choose wire product</option>{b.products.map(p=><option key={p.id} value={p.id}>{p.name} · {p.thickness}</option>)}</select></Field><SaleQuantityInput kgValue={q} onChangeKg={setQ} testIdPrefix="pos" ratePerKg={selected?.salePrice}/><Button onClick={addLine} testId="button-add-to-cart"><Plus size={16}/> Add</Button></div>{selected&&<div className="mt-4 flex items-center justify-between rounded-lg bg-emerald-50 px-4 py-3 text-xs"><span className="font-semibold text-emerald-900">{selected.name} · {money(selected.salePrice*KG_PER_TON)}/Ton</span><span className="text-emerald-700">Available: {qty(inventoryStats(b,selected).remaining)}</span></div>}<div className="mt-7 border-t pt-5"><div className="mb-3 flex items-center justify-between"><h3 className="font-display font-bold">Current ticket</h3><span className="text-xs text-slate-400">{cart.length} line{cart.length===1?"":"s"}</span></div>{cart.length?<div className="space-y-2">{cart.map((x,i)=>{const p=b.products.find(z=>z.id===x.productId)!;return <div key={x.productId} className="flex items-center justify-between rounded-lg border bg-slate-50/70 p-3" data-testid={`cart-line-${x.productId}`}><div><div className="text-sm font-bold">{p.name}</div><div className="text-xs text-slate-500">{qty(x.quantityKg)} × {money(x.saleRate*KG_PER_TON)}/Ton</div></div><div className="flex items-center gap-3"><div className="text-right"><div className="text-sm font-bold">{money(x.quantityKg*x.saleRate)}</div><div className="text-[11px] text-emerald-700">+{money(x.quantityKg*(x.saleRate-x.purchaseRate))}</div></div><Button variant="danger" onClick={()=>setCart(c=>c.filter((_,j)=>j!==i))} testId={`button-remove-cart-${x.productId}`}><X size={14}/></Button></div></div>})}</div>:<Empty icon={ShoppingCart} title="Ticket is empty" description="Choose a product above to start the sale."/>}</div></div><div className="data-card span-5 p-5"><div className="kicker">Settlement</div><h3 className="mt-1 font-display text-lg font-bold">Sale summary</h3><div className="mt-5 space-y-3 border-b pb-5 text-sm"><div className="flex justify-between text-slate-500"><span>Subtotal</span><b className="text-slate-800">{money(totals.sub)}</b></div><div className="flex justify-between text-slate-500"><span>Gross profit</span><b className="text-emerald-700">{money(totals.profit)}</b></div></div><div className="mt-5 space-y-4"><Field label="Customer (required for credit)"><select className="input-field" value={customer} onChange={e=>setCustomer(e.target.value)} data-testid="select-pos-customer"><option value="">Walk-in customer</option>{b.customers.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></Field>{!customer&&<div className="grid gap-3 rounded-lg border border-dashed border-slate-200 p-3"><p className="text-[11px] font-semibold text-slate-500">Walk-in customer details (optional)</p><input className="input-field text-xs" value={walkInName} onChange={e=>setWalkInName(e.target.value)} placeholder="Customer name" data-testid="input-walkin-name"/><input className="input-field text-xs" value={walkInPhone} onChange={e=>setWalkInPhone(e.target.value)} placeholder="Phone number" data-testid="input-walkin-phone"/><input className="input-field text-xs" value={walkInAddress} onChange={e=>setWalkInAddress(e.target.value)} placeholder="Address" data-testid="input-walkin-address"/></div>}<Field label="Payment method"><div className="grid grid-cols-2 gap-2">{(["cash","credit"] as const).map(x=><button key={x} onClick={()=>setMethod(x)} className={`rounded-lg border px-3 py-2 text-sm font-bold ${method===x?"border-emerald-600 bg-emerald-50 text-emerald-800":"border-slate-200 text-slate-500"}`} data-testid={`button-payment-${x}`}>{x==="cash"?"Cash":"Credit"}</button>)}</div></Field><Field label="Sale date"><AutoDate date={saleDate} onChange={setSaleDate} testId="input-pos-sale-date"/></Field>{method==="credit"&&<Field label="Paid today"><input className="input-field" type="number" value={paid} onChange={e=>setPaid(e.target.value)} placeholder="0" data-testid="input-pos-paid"/></Field>}<div className="rounded-xl bg-slate-900 p-4 text-white"><div className="text-xs text-white/60">Amount due</div><div className="mt-1 font-display text-2xl font-bold">{money(totals.sub)}</div>{method==="credit"&&<div className="mt-1 text-xs text-teal-300">Remaining credit: {money(Math.max(0,totals.sub-(Number(paid)||0)))}</div>}</div><Button onClick={complete} className="w-full py-3" testId="button-complete-sale"><Check size={16}/> Complete sale</Button></div></div></div><Toast message={toast} onClose={()=>setToast("")}/></>}
...
function SaleEditModal({sale,onClose,onSaved}:{sale:any;onClose:()=>void;onSaved:(msg:string)=>void}){
  ...
  const addItem=()=>{
    ...
    setLines(ls=>[...ls,{productId:addProduct.id,quantityKg:Number(addQty)*KG_PER_TON,saleRate:addProduct.salePrice,purchaseRate:addProduct.purchasePrice,date:addDate}]);
  ...
  return <Modal title={`Edit Sale #${String(sale.id).padStart(4,"0")}`} eyebrow="Sale record" onClose={onClose}>
    ...
    <div className="mt-6 border-t pt-4">
      ...
      <div className="mt-3 space-y-2">
        {lines.map((l,i)=>{const p=b.products.find(x=>x.id===l.productId);return <div key={i} className="grid grid-cols-2 items-center gap-2 rounded-lg border bg-slate-50/70 p-3 text-xs md:grid-cols-[1.4fr_1fr_1fr_1fr_auto]" data-testid={`edit-sale-line-${i}`}>
          <div className="font-bold text-slate-700 md:col-span-1 col-span-2">{p?.name||"Product"}</div>
          <label className="block"><span className="mb-0.5 block text-[10px] font-bold text-slate-400">Qty (Ton)</span><input type="number" min="0" step="any" className="input-field !py-1.5 text-xs" value={l.quantityKg?Number((l.quantityKg/KG_PER_TON).toFixed(4)):""} onChange={e=>updateLine(i,{quantityKg:Number(e.target.value||0)*KG_PER_TON})} data-testid={`input-edit-sale-line-qty-${i}`}/></label>
          <label className="block"><span className="mb-0.5 block text-[10px] font-bold text-slate-400">Rate /Ton</span><input type="number" min="0" step="any" className="input-field !py-1.5 text-xs" value={l.saleRate?Number((l.saleRate*KG_PER_TON).toFixed(4)):""} onChange={e=>updateLine(i,{saleRate:Number(e.target.value||0)/KG_PER_TON})} data-testid={`input-edit-sale-line-rate-${i}`}/></label>
          <label className="block"><span className="mb-0.5 block text-[10px] font-bold text-slate-400">Date</span><input type="date" className="input-field !py-1.5 text-xs" value={l.date||sale.date} onChange={e=>updateLine(i,{date:e.target.value})} data-testid={`input-edit-sale-line-date-${i}`}/></label>
          <button type="button" onClick={()=>removeLine(i)} className="justify-self-end text-red-600 md:self-end md:pb-1.5" aria-label="Remove item" data-testid={`button-edit-sale-remove-line-${i}`}><Trash2 size={14}/></button>
        </div>})}
      </div>
    </div>
...
function SupplierCard({s}:{s:any}){
  ...
  const total=ps.reduce((a,x)=>a+x.quantityKg*x.purchaseRate,0),paid=ps.reduce((a,x)=>a+x.paidAmount,0)+b.supplierPayments.filter(x=>x.supplierId===s.id&&!x.purchaseId).reduce((a,x)=>a+x.amount,0),due=total-paid;
...
function Suppliers(){const b=useBusiness();const [show,setShow]=useState(false);const [form,setForm]=useState<any>({name:"",company:"",phone:"",address:"",notes:""});const [search,setSearch]=useState("");const [toast,setToast]=useState("");const save=()=>{if(!form.name){setToast("Supplier name is required");return}b.add("suppliers",form);setShow(false);setForm({name:"",company:"",phone:"",address:"",notes:""});setToast("Supplier added successfully")};
 const blankPurchase={supplierId:"",invoiceNumber:"",date:today,productId:"",quantityKg:"",purchaseRate:"",paidAmount:"",notes:""};
 ...
 const purchaseTotal=Number(purchaseForm.quantityKg||0)*Number(purchaseForm.purchaseRate||0);
 const savePurchase=async()=>{if(!purchaseForm.supplierId){setToast("Select a supplier");return}if(!purchaseForm.productId||Number(purchaseForm.quantityKg)<=0||Number(purchaseForm.purchaseRate)<=0){setToast("Complete product, quantity and rate");return}const r=await b.addPurchase({id:0,supplierId:Number(purchaseForm.supplierId),invoiceNumber:purchaseForm.invoiceNumber,date:purchaseForm.date,productId:Number(purchaseForm.productId),quantityKg:Number(purchaseForm.quantityKg),purchaseRate:Number(purchaseForm.purchaseRate),paidAmount:Number(purchaseForm.paidAmount||0),notes:purchaseForm.notes});setToast(r.message);if(r.ok){setShowPurchase(false);setPurchaseForm(blankPurchase)}};
 ...
 {showPurchase&&<Modal title="Record purchase" eyebrow="Vendor network" onClose={()=>setShowPurchase(false)}><div className="grid gap-4 md:grid-cols-2"><Field label="Supplier" wide><select className="input-field" value={purchaseForm.supplierId} onChange={e=>setPurchaseForm({...purchaseForm,supplierId:e.target.value})} data-testid="select-purchase-supplier"><option value="">Select supplier</option>{b.suppliers.map(s=><option key={s.id} value={s.id}>{s.name}{s.company?` · ${s.company}`:""}</option>)}</select></Field><Field label="Invoice number"><input className="input-field" value={purchaseForm.invoiceNumber} onChange={e=>setPurchaseForm({...purchaseForm,invoiceNumber:e.target.value})} data-testid="input-top-purchase-invoice"/></Field><Field label="Purchase date"><AutoDate date={purchaseForm.date} onChange={v=>setPurchaseForm({...purchaseForm,date:v})} testId="input-top-purchase-date"/></Field><Field label="Product"><select className="input-field" value={purchaseForm.productId} onChange={e=>{const p=b.products.find(x=>x.id===Number(e.target.value));setPurchaseForm({...purchaseForm,productId:e.target.value,purchaseRate:p?.purchasePrice||""})}} data-testid="select-top-purchase-product"><option value="">Select product</option>{b.products.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></Field><QuantityInput kgValue={purchaseForm.quantityKg} onChangeKg={v=>setPurchaseForm({...purchaseForm,quantityKg:v})} testIdPrefix="top-purchase"/><PriceInput perKgValue={purchaseForm.purchaseRate} onChangePerKg={v=>setPurchaseForm({...purchaseForm,purchaseRate:v})} testIdPrefix="top-purchase" label="Purchase rate"/><Field label="Paid amount"><input type="number" className="input-field" value={purchaseForm.paidAmount} onChange={e=>setPurchaseForm({...purchaseForm,paidAmount:e.target.value})} data-testid="input-top-purchase-paid"/></Field><Field label="Notes" wide><input className="input-field" value={purchaseForm.notes} onChange={e=>setPurchaseForm({...purchaseForm,notes:e.target.value})} data-testid="input-top-purchase-notes"/></Field></div><div className="mt-5 flex items-center justify-between rounded-lg bg-emerald-50 p-4"><span className="text-sm font-semibold text-emerald-900">Total purchase cost</span><b className="font-display text-xl text-emerald-800">{money(purchaseTotal)}</b></div><div className="mt-6 flex justify-end gap-2"><Button variant="secondary" onClick={()=>setShowPurchase(false)}>Cancel</Button><Button onClick={savePurchase} testId="button-save-purchase-top"><Check size={15}/> Save purchase</Button></div></Modal>}
 <Toast message={toast} onClose={()=>setToast("")}/></>}
...
function SupplierProfile(){
  ...
  const blankPurchase={invoiceNumber:"",date:today,productId:"",quantityKg:"",purchaseRate:"",paidAmount:"",notes:""};
  ...
  const total=ps.reduce((a,x)=>a+x.quantityKg*x.purchaseRate,0),paidOnInvoices=ps.reduce((a,x)=>a+x.paidAmount,0),paidSeparately=payments.filter(x=>!x.purchaseId).reduce((a,x)=>a+x.amount,0),paid=paidOnInvoices+paidSeparately,due=total-paid,kgTotal=ps.reduce((a,x)=>a+x.quantityKg,0);
  
  const payableInvoices=ps.filter(x=>x.quantityKg*x.purchaseRate-x.paidAmount>0.009);
  const payInvoice=payInvoiceId?ps.find(x=>x.id===Number(payInvoiceId)):null;
  const payInvoiceRemaining=payInvoice?payInvoice.quantityKg*payInvoice.purchaseRate-payInvoice.paidAmount:0;
  const totalCalc=Number(form.quantityKg||0)*Number(form.purchaseRate||0);
  const openNew=()=>{setEditingPurchase(null);setForm(blankPurchase);setShow(true)};
  const openEdit=(x:any)=>{setEditingPurchase(x);setForm({invoiceNumber:x.invoiceNumber,date:x.date,productId:String(x.productId),quantityKg:String(x.quantityKg),purchaseRate:String(x.purchaseRate),paidAmount:String(x.paidAmount),notes:x.notes});setShow(true)};
  const save=async()=>{if(!form.productId||Number(form.quantityKg)<=0||Number(form.purchaseRate)<=0){setToast("Complete product, quantity and rate");return}const payload={supplierId,invoiceNumber:form.invoiceNumber,date:form.date,productId:Number(form.productId),quantityKg:Number(form.quantityKg),purchaseRate:Number(form.purchaseRate),paidAmount:Number(form.paidAmount||0),notes:form.notes};if(editingPurchase){b.update("purchases",editingPurchase.id,payload);setToast("Purchase updated")}else{const r=await b.addPurchase({...payload,id:0});setToast(r.message)}setShow(false);setEditingPurchase(null);setForm(blankPurchase)};
  const removePurchase=(pid:number)=>{if(confirm("Delete this purchase record? Stock will be adjusted accordingly.")){b.remove("purchases",pid);setToast("Purchase deleted")}};
  ...
  const savePayment=async()=>{
    ...
    if(payInvoiceId){
      ...
      b.recordSupplierPayment(supplierId,Number(payAmount),payNote||"Payment to supplier",payDate);
  ...
  const removePayment=(pid:number)=>{
    ...
    if(confirm("Delete this payment record?")){
      if(payment?.purchaseId){
        ...
        if(inv) b.update("purchases",inv.id,{paidAmount:Math.max(0,inv.paidAmount-payment.amount)});
      }
      b.remove("supplierPayments",pid);setToast("Payment deleted");
    }
  };
  const saveSupplier=()=>{if(!supplierForm.name){setToast("Supplier name is required");return}b.update("suppliers",supplierId,supplierForm);setEditSupplier(false);setToast("Supplier updated")};
  ...
  <div className="content-grid">
   <div className="data-card span-4 p-5">
    ...
    <Button variant="danger" className="mt-5 w-full" onClick={()=>{if(confirm("Delete this supplier? Purchase history will remain in reports.")){b.remove("suppliers",supplierId);setToast("Supplier deleted")}}} testId="button-delete-supplier">Delete supplier</Button>
   </div>
  ...
  <div className="content-grid mt-4">
   <div className="data-card span-4 p-5">
    ...
    <div className="mt-4 space-y-3">
     <Field label="Apply to invoice"><select className="input-field" value={payInvoiceId} onChange={e=>setPayInvoiceId(e.target.value)} data-testid="select-supplier-payment-invoice"><option value="">General (not tied to an invoice)</option>{payableInvoices.map(x=><option key={x.id} value={x.id}>{x.invoiceNumber||`PUR-${x.id}`} · {dateLabel(x.date)} · Due {money(x.quantityKg*x.purchaseRate-x.paidAmount)}</option>)}</select></Field>
  ...
  {show&&<Modal title={editingPurchase?"Edit purchase":"Record purchase"} eyebrow={supplier.name} onClose={()=>setShow(false)}><div className="grid gap-4 md:grid-cols-2"><Field label="Invoice number"><input className="input-field" value={form.invoiceNumber} onChange={e=>setForm({...form,invoiceNumber:e.target.value})} data-testid="input-purchase-invoice"/></Field><Field label="Purchase date"><AutoDate date={form.date} onChange={v=>setForm({...form,date:v})} testId="input-purchase-date"/></Field><Field label="Product"><select className="input-field" value={form.productId} onChange={e=>{const p=b.products.find(x=>x.id===Number(e.target.value));setForm({...form,productId:e.target.value,purchaseRate:p?.purchasePrice||""})}} data-testid="select-purchase-product"><option value="">Select product</option>{b.products.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></Field><QuantityInput kgValue={form.quantityKg} onChangeKg={v=>setForm({...form,quantityKg:v})} testIdPrefix="purchase"/><PriceInput perKgValue={form.purchaseRate} onChangePerKg={v=>setForm({...form,purchaseRate:v})} testIdPrefix="purchase" label="Purchase rate"/><Field label="Paid amount"><input type="number" className="input-field" value={form.paidAmount} onChange={e=>setForm({...form,paidAmount:e.target.value})} data-testid="input-purchase-paid"/></Field><Field label="Notes" wide><input className="input-field" value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} data-testid="input-purchase-notes"/></Field></div><div className="mt-5 flex items-center justify-between rounded-lg bg-emerald-50 p-4"><span className="text-sm font-semibold text-emerald-900">Total purchase cost</span><b className="font-display text-xl text-emerald-800">{money(totalCalc)}</b></div><div className="mt-6 flex justify-end gap-2"><Button variant="secondary" onClick={()=>setShow(false)}>Cancel</Button><Button onClick={save} testId="button-save-purchase"><Check size={15}/> Save purchase</Button></div></Modal>}
  {viewingPurchase&&<Modal title={viewingPurchase.invoiceNumber||`PUR-${viewingPurchase.id}`} eyebrow="Purchase invoice" onClose={()=>{setViewPurchaseId(null);setInvPayAmount("");setInvPayNote("")}}>
    {(()=>{const inv=purchaseInvoiceData(b,viewingPurchase,supplier);const invBalance=viewingPurchase.quantityKg*viewingPurchase.purchaseRate-viewingPurchase.paidAmount;return <div className="space-y-5">
...
function Expenses(){
 ...
 const rangeStart=range==="day"?today:range==="week"?new Date(Date.now()-6*86400000).toISOString().slice(0,10):range==="month"?new Date(new Date().getFullYear(),new Date().getMonth(),1).toISOString().slice(0,10):range==="year"?`${new Date().getFullYear()}-01-01`:"0000-00-00";
 const periodExpenses=b.expenses.filter(x=>x.date>=rangeStart);
 const save=()=>{if(!form.title||Number(form.amount)<=0){setToast("Enter a title and positive amount");return}const item={...form,amount:Number(form.amount)};editing?b.update("expenses",editing.id,item):b.add("expenses",item);setShow(false);setEditing(null);setForm({title:"",category:cats[0]||"",amount:"",date:today,description:""});setToast("Expense saved successfully")};
...
function Credit(){
  ...
  const save=()=>{if(!form.name){setToast("Customer name is required");return}b.add("customers",form);setShow(false);setForm({name:"",phone:"",address:""});setToast("Customer added successfully")};
...
function CustomerProfile(){
  ...
  const save=async()=>{
    ...
    if(payInvoiceId){
      ...
      b.recordPayment(customerId,Number(amount),note||(isAdvance?"Advance payment":"Credit payment"),payDate);
      setToast(isAdvance?"Advance payment recorded — it will apply to future purchases":"Credit payment recorded successfully");
    }
    setAmount("");setNote("");setPayDate(today);setPayInvoiceId("");
  };
  const saveCustomer=()=>{if(!customerForm.name){setToast("Customer name is required");return}b.update("customers",customerId,customerForm);setEditCustomer(false);setToast("Customer updated")};
  const doDeleteSale=()=>{if(!confirmDeleteSale)return;b.removeSale(confirmDeleteSale.id);setToast("Invoice deleted");setConfirmDeleteSale(null)};
  ...
  <div className="content-grid">
   <div className="data-card span-4 p-5">
    ...
    <Button variant="danger" className="mt-5 w-full" onClick={()=>{if(confirm("Delete this customer? Sale history will remain in reports.")){b.remove("customers",customerId);setToast("Customer deleted")}}} testId="button-delete-customer">Delete customer</Button>
   </div>
  ...
  <div className="data-card span-12 mt-4"><div className="border-b p-5"><h3 className="font-display font-bold">Purchase / invoice history</h3><p className="text-xs text-slate-500">Every sale made to {customer.name}, with products, quantities and a downloadable invoice.</p></div>
   {sales.length?<div className="table-wrap"><table className="data-table"><thead><tr><th>Sale</th><th>Date</th><th>Products bought</th><th>Payment</th><th className="text-right">Amount</th><th className="text-right">Action</th></tr></thead><tbody>{salesPager.pageItems.map(s=><tr key={s.id} data-testid={`row-customer-sale-${s.id}`}><td className="font-bold">Sale #{String(s.id).padStart(4,"0")}</td><td>{dateLabel(s.date)}</td><td>{s.lines.map((l,i)=>{const p=b.products.find(z=>z.id===l.productId);return <div key={i} className="text-slate-600">{p?.name||"Wire product"} · {qty(l.quantityKg)}</div>})}</td><td><span className={`status ${s.paymentMethod==="credit"?"status-low":"status-good"}`}>{s.paymentMethod}</span></td><td className="text-right font-bold">{money(s.subtotal)}</td><td className="text-right"><div className="flex justify-end gap-1"><Link href={`/sales/${s.id}`} className="btn btn-ghost !p-2" title="View invoice" data-testid={`link-view-customer-sale-${s.id}`}><FileText size={14}/></Link><Button variant="ghost" onClick={()=>setEditingSale(s)} testId={`button-edit-customer-sale-${s.id}`}><Pencil size={14}/></Button><Button variant="danger" onClick={()=>setConfirmDeleteSale(s)} testId={`button-delete-customer-sale-${s.id}`}><Trash2 size={14}/></Button></div></td></tr>)}</tbody></table><Pager page={salesPager.page} totalPages={salesPager.totalPages} onChange={salesPager.setPage} testId="customer-sales"/></div>:<Empty icon={ShoppingCart} title="No purchases yet" description="Sales made to this customer will appear here."/>}
  </div>
  <div className="data-card span-12 mt-4">
...
function Reports(){
 const b=useBusiness(); const [range,setRange]=useState("month"); const [from,setFrom]=useState(new Date(new Date().getFullYear(),new Date().getMonth(),1).toISOString().slice(0,10)); const [to,setTo]=useState(today);
 const start=range==="today"?today:range==="yesterday"?yesterday:range==="week"?new Date(Date.now()-7*86400000).toISOString().slice(0,10):range==="month"?new Date(new Date().getFullYear(),new Date().getMonth(),1).toISOString().slice(0,10):range==="custom"?from:"2000-01-01";
 const end=range==="today"?today:range==="yesterday"?yesterday:range==="custom"?to:today;
 const sales=b.sales.filter(x=>x.date>=start&&x.date<=end); const expenses=b.expenses.filter(x=>x.date>=start&&x.date<=end);
 const saleTotal=sales.reduce((a,x)=>a+x.subtotal,0),gross=sales.reduce((a,x)=>a+x.grossProfit,0),kg=sales.flatMap(x=>x.lines).reduce((a,x)=>a+x.quantityKg,0),expenseTotal=expenses.reduce((a,x)=>a+x.amount,0);
...
function Settings(){const b=useBusiness();const [form,setForm]=useState(b.settings);const [toast,setToast]=useState("");const save=()=>{b.updateSettings(form);setToast("Settings saved")};
...
function Sales(){
 ...
 return <><PageHead eyebrow="Sale records" title="Sales" description="Every completed sale, with a full record you can open any time." action={<Link href="/pos" className="btn btn-primary" data-testid="link-new-sale-from-sales"><Plus size={16}/> New sale</Link>}/>
  ...
  <div className="table-wrap"><table className="data-table"><thead><tr><th>Sale</th><th>Date</th><th>Customer</th><th>Payment</th><th className="text-right">Amount</th><th className="text-right">Action</th></tr></thead><tbody>{pageItems.map(s=><tr key={s.id} data-testid={`row-sale-record-${s.id}`}><td className="font-bold">Sale #{String(s.id).padStart(4,"0")}</td><td>{dateLabel(s.date)}</td><td>{saleCustomerLabel(b,s)}</td><td><span className={`status ${s.paymentMethod==="credit"?"status-low":"status-good"}`}>{s.paymentMethod}</span></td><td className="text-right font-bold">{money(s.subtotal)}</td><td className="text-right"><div className="flex justify-end gap-1"><Link href={`/sales/${s.id}`} className="btn btn-ghost !p-2" title="View invoice" data-testid={`link-view-sale-${s.id}`}><FileText size={14}/></Link><Button variant="ghost" onClick={()=>setEditing(s)} testId={`button-edit-sale-${s.id}`}><Pencil size={14}/></Button><Button variant="danger" onClick={()=>setConfirmDelete(s)} testId={`button-delete-sale-${s.id}`}><Trash2 size={14}/></Button></div></td></tr>)}</tbody></table>{!rows.length&&<Empty icon={ShoppingCart} title="No sales found" description="Adjust the period or search, or complete a new sale."/>}<Pager page={page} totalPages={totalPages} onChange={setPage} testId="sales"/></div></div>
...
function SaleDetail(){
 ...
 const doDelete=()=>{b.removeSale(sale.id);setLoc("/sales")};
 return <><PageHead eyebrow="Sale record" title={`Sale #${String(sale.id).padStart(4,"0")}`} description={`Recorded on ${dateLabel(sale.date)}`} action={<div className="flex flex-wrap gap-2"><Link href="/sales" className="btn btn-secondary" data-testid="link-back-to-sales">Back to sales</Link><Button variant="secondary" onClick={()=>setEditing(true)} testId="button-edit-this-sale"><Pencil size={14}/> Edit sale</Button><Button variant="danger" onClick={()=>setConfirmDelete(true)} testId="button-delete-this-sale"><Trash2 size={14}/> Delete</Button></div>}/>
  <div className="content-grid">
   ...
   <div className="data-card span-4 p-5"><div className="kicker">Customer</div><h3 className="mt-1 font-display text-lg font-bold">{saleCustomerLabel(b,sale)}</h3>
    ...
    <div className="mt-6 space-y-3 border-t pt-4 text-sm"><div className="flex justify-between text-slate-500"><span>Payment method</span><span className={`status ${sale.paymentMethod==="credit"?"status-low":"status-good"}`}>{sale.paymentMethod}</span></div><div className="flex justify-between text-slate-500"><span>Gross profit</span><b className="text-emerald-700">{money(sale.grossProfit)}</b></div><div className="flex justify-between text-slate-500"><span>Total paid so far</span><b className="text-slate-800">{money(totalPaid)}</b></div>{remaining>0&&<div className="flex justify-between text-slate-500"><span>Balance remaining</span><b className="text-amber-700">{money(remaining)}</b></div>}</div>
   </div>
  </div>
  ...
  return <div className="app-shell flex min-h-dvh items-center justify-center p-4">
    <div className="w-full max-w-md">
      <div className="mb-6 flex flex-col items-center gap-2 text-center">
        <BrandLogo className="h-12 w-12 rounded-xl" iconSize={24}/>
      ...
      <form onSubmit={submit} className="data-card p-6" data-testid="form-login">
        ...
        <div className="mt-5 space-y-4">
          <Field label="Email"><input type="email" className="input-field" value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@example.com" autoComplete="email" data-testid="input-login-email"/></Field>
        ...
        <Button onClick={submit} className="mt-5 w-full" testId="button-login-submit"><LockKeyhole size={15}/> Sign in</Button>
      </form>
    </div>
    <Toast message={toast} onClose={()=>setToast("")}/>
  </div>;
...
function UserManagementInner(){
  ...
  const blankForm={password:"",name:"",email:"",phone:"",role:"admin" as Role};
  ...
  <div className="data-card">
    <div className="table-wrap"><table className="data-table"><thead><tr><th>User</th><th>Role</th><th>Plan</th><th>Status</th><th className="text-right">Actions</th></tr></thead><tbody>
      {admins.length===0?<tr><td colSpan={5} className="text-center text-slate-500 py-8">No users yet — add one to get started</td></tr>:admins.map(u=>
        <UserRow key={u.username} u={u} auth={auth} setToast={setToast} onEdit={()=>setEditing(u)}/>
      )}
    </tbody></table></div>
  </div>
  ...
  {editing&&<EditUserModal user={editing} auth={auth} onClose={()=>setEditing(null)} setToast={setToast}/>}
  <Toast message={toast} onClose={()=>setToast("")}/>
  </>;
}
function EditUserModal({user,auth,onClose,setToast}:{user:AuthUser;auth:ReturnType<typeof useAuth>;onClose:()=>void;setToast:(s:string)=>void}){
  ...
  const save=()=>{
    ...
    const emailTaken=auth.users.some(x=>x.username!==user.username&&x.email.toLowerCase()===form.email.trim().toLowerCase());
    if(emailTaken){setToast("Another account already uses this email");return}
    const data:Partial<AuthUser>={name:form.name.trim(),email:form.email.trim(),phone:form.phone,role:form.role};
    if(form.password.trim()) data.password=form.password.trim();
    auth.updateUser(user.username,data);
    setToast("User updated");
    onClose();
  };
  return <Modal title="Edit user" eyebrow="User management" onClose={onClose}>
    <div className="grid gap-4 sm:grid-cols-2">
      <Field label="Full name"><input className="input-field" value={form.name} onChange={e=>setForm({...form,name:e.target.value})} data-testid="input-edit-user-name"/></Field>
      <Field label="Email"><input type="email" className="input-field" value={form.email} onChange={e=>setForm({...form,email:e.target.value})} data-testid="input-edit-user-email"/></Field>
      <Field label="Phone"><input className="input-field" value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} data-testid="input-edit-user-phone"/></Field>
      <Field label="Access level"><select className="input-field" value={form.role} onChange={e=>setForm({...form,role:e.target.value as Role})} data-testid="select-edit-user-role"><option value="admin">Admin</option><option value="superadmin">Super Admin</option></select></Field>
      <Field label="New password" wide><input type="password" className="input-field" value={form.password} onChange={e=>setForm({...form,password:e.target.value})} placeholder="Leave blank to keep current password" data-testid="input-edit-user-password"/></Field>
    </div>
    <Button onClick={save} className="mt-5 w-full" testId="button-save-user"><Check size={15}/> Save changes</Button>
  </Modal>;
}
function UserRow({u,auth,setToast,onEdit}:{u:AuthUser;auth:ReturnType<typeof useAuth>;setToast:(s:string)=>void;onEdit:()=>void}){
  const [planPick,setPlanPick]=useState<PlanId>("month");
  const [customDays,setCustomDays]=useState("30");
  const expired=isPlanExpired(u);
  const assign=()=>{
    const days=planPick==="custom"?Number(customDays||30):undefined;
    auth.assignPlan(u.username,planPick,days);
    setToast(`${planPick==="custom"?`Custom plan (${days} days)`:PLAN_LABELS[planPick]} assigned to ${u.name}`);
  };
  return <tr>
    <td><div className="font-bold">{u.name}</div><div className="text-xs text-slate-500">{u.email}</div></td>
    ...
    <td>
      <div className="flex items-center gap-2">
        <select className="input-field w-36 text-xs" value={planPick} onChange={e=>setPlanPick(e.target.value as PlanId)} data-testid={`select-plan-${u.username}`}>
          {(["demo","month","year","custom"] as const).map(p=><option key={p} value={p}>{PLAN_LABELS[p]}</option>)}
        </select>
        {planPick==="custom"&&<input type="number" min="1" className="input-field w-20 text-xs" value={customDays} onChange={e=>setCustomDays(e.target.value)} placeholder="Days" data-testid={`input-custom-days-${u.username}`}/>}
        <Button variant="secondary" onClick={assign} testId={`button-assign-plan-${u.username}`}>Assign</Button>
      </div>
    ...
    <td className="text-right"><div className="inline-flex items-center gap-1">
      <Button variant="ghost" onClick={onEdit} testId={`button-edit-user-${u.username}`}><Pencil size={14}/></Button>
...
function AdminPaymentsInner(){
  ...
  <div className="content-grid">
    <div className="data-card span-5 p-5">
      ...
      <div className="mt-5 space-y-4">
        <Field label="Admin account"><select className="input-field" value={selected} onChange={e=>setSelected(e.target.value)} data-testid="select-payment-user">{admins.length===0?<option value="">No admin accounts</option>:admins.map(u=><option key={u.username} value={u.username}>{u.name} ({u.email})</option>)}</select></Field>
...
function AuthGate(){
  const auth=useAuth();
  if(auth.initializing) return <div className="grid min-h-dvh place-items-center bg-slate-50 text-sm font-semibold text-slate-500">Loading…</div>;
